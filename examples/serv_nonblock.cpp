...
int main()
{
    ...
    {
        std::cerr << server.getStatusMessage(status) << std::endl;
        return 1;
    }
    ...
    while (true)
    {
        ...
        if (accept_status == sck::Socket::Status::Good)
        {
            ...
            if (recv_status == sck::Socket::Status::Good)
            {
                ...
                {
                    std::cerr << "Failed to send echo: " << client.getStatusMessage() << std::endl;
                }
                else
                {
            ...
            {
                std::cerr << "Receive error: " << static_cast<int>(recv_status) << std::endl;
            }
        ...
        {
            std::cerr << "Accept error: " << static_cast<int>(accept_status) << std::endl;
        }
    }
}